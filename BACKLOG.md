...
- [ ] Small answers for the agents that operate the fleet: `CLIENT LIST` (who is connected — worker, bridge, CLI), `tritium-cli where <key>` (which nodes hold it, TTL on each), `tritium-msg status` (bridge sessions, last message, latency). Done so far: `tritium-cli info [SECTION]` (v0.16.4), `CLIENT LIST`/`SETNAME`/`GETNAME` + `tritium-cli clients` (v0.17.2; mubs clients should pass a client name so the list reads worker/bridge/gateway), `tritium-cli where KEY` (v0.17.3). Left: `tritium-msg status` — 2026-09-08
