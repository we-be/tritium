...
- **Outages take the path they already took.** A linked peer that stops
  ...
  (`internal/replica/store.go`) needed no changes at all.
...
Nor should the cloud node order writes. With `ELECTRONEGATIVITY=0` it never
owns a key: a write the gateway hands it is forwarded down a link to a home
node, the home nodes never forward to it, and it is not marked leader even
though it seeded nothing. Key order is decided on the LAN, where the writers
are; the cloud node only carries replicas. (A node that does have a weight
never forwards to a peer that linked to it, since that peer's fan-out reaches
only what it can dial; a node of weight 0 hands every write to its peers and
leaves their reach to them — on a fleet whose home nodes reach each other,
that is the point.)
...
ELECTRONEGATIVITY=0          # carries replicas, orders nothing: never a key's owner, never leader
